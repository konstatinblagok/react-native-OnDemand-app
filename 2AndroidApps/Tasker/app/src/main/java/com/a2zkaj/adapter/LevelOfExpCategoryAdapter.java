 ...
 */
public class LevelOfExpCategoryAdapter extends BaseAdapter {
    private ArrayList<ParentCategorypojo> parentCategorypojoArrayList;
    private Context myContext;

    public LevelOfExpCategoryAdapter(Context aContext, ArrayList<ParentCategorypojo> parentCat_List) {
    ...
    public int getPositionForItem(String parentID) {
        ...
        for (int i = 0; i < parentCategorypojoArrayList.size(); i++) {
            if (parentCategorypojoArrayList.get(i).getParentCategory_name().equalsIgnoreCase(parentID)) {
                pos = i;
                break;
            }
        }
