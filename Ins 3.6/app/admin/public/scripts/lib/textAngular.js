...
if(_browserDetect.ie > 8 || _browserDetect.ie === undefined){
	...
	if(!sheet){
		...
		sheet = (function() {
			...
			// Add the <style> element to the page, add as first so the css can be overridden by custom stylesheets
	...
	function transformLegacyStyles(html){
		...
		while(match = styleElementMatch.exec(html)){
			...
			if(angular.isString(styleVal) && styleRegex.test(styleVal)){
				...
				// recursively find more legacy css in html before this tag and after the previous match (if any)
				...
				angular.extend(scope, angular.copy(taOptions), {
					...
					wrapSelection: function(command, opt, isSelectableElementTool){
						if(command.toLowerCase() === "undo"){
							...
							// refocus on the shown display element, this fixes a display bug when using :focus css to outline the box.
